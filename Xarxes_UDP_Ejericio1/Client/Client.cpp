...
void Send(Cabezera cabezera, std::string valor, sf::UdpSocket &socket, Client server, double clientSalt) //A un cliente en concreto
{
	std::string cab;
	cab = std::to_string(cabezera);

	sf::Packet pack;
	cab = cab + "_" + "<" + std::to_string(clientSalt) + ">" + "<" + std::to_string(server.salt) + ">"; //Cabezera + salts
...
int main()
{
	...
	while (!isConnected)
	{
		...
		if ((clock() - timer) / CLOCKS_PER_SEC >= latency)
		{
			c = Cabezera::HELLO;
			m = userName;
			Send(c, m, socket, server, clientSalt); // HELLO_<2789492><0>Pau
		...
		if (c == Cabezera::CHALLENGE) //Posible suma
		{
			...
			m = std::to_string(a + b);
			Send(c, m, socket, server, clientSalt);	
		}
